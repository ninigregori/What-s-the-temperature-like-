...
$(document).ready(function() {
  ...
  //ipinfo to get geolocation since navigator.geolocation is deprecated
  //over unsecure http
  $.get("http://ip-api.com/json/?callback=?", function(response) {
    getWeather(response.lat, response.lon);
    loadCity(response.city);
  }, "jsonp");
...
function getWeather(lat,lon) {
  $.ajax({
    url: "http://api.openweathermap.org/data/2.5/weather?lat=" + lat + "&lon=" + lon + "&APPID=a700590fb163ef17e15ff4a80b289444",
    accept: "text/json",
    success: function(response) {
      loadIcon(response.weather);
...
function loadIcon(weather) {
  var prefix = 'wi wi-';
  var code = weather[0].id;
  var icon = weatherCodes[code].icon;
...
function loadTemp() {
  $('#temp').html(tempString(TEMP));
}

var tempString = function(temp) {
  return TEMPERATURE == "fahrenheit" ?
    Math.round((temp * 9 / 5) - 459.67) + " &#8457" :
    Math.round(temp - 273.15) + " &#8451";
}
